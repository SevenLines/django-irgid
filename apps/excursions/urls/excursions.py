...
from excursions.views import MainPageView, CategoryView, ExcursionItemView, ExcursionGalleryItemView, \
    ExcursionGalleryIndexView, ExcursionRemove, CategoryRemove

__author__ = 'm'

urlpatterns = i18n_patterns('excursions.views',
    url(r'c/(?P<pk>(\d+))/remove/$', CategoryRemove.as_view(), name='category_remove'),
    ...
    url(r'(?P<pk>(\d+))/remove/$', ExcursionRemove.as_view(), name='remove'),
    url(r'(?P<pk>(\d+))/$', ExcursionItemView.as_view(), name='item'),

    url(r'$', MainPageView.as_view(), name='index'),

    # для совместимости со старой версией ссылок
    url(r'excursion/(?P<pk>(\d+))/remove/$', ExcursionRemove.as_view()),
    ...
    url(r'category/(?P<pk>(\d+))/remove/$', CategoryRemove.as_view()),
